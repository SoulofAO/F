...
from .SmartSite.apps.osn.models import Comment,Priloshenie,ButtonFind
...
def get_content(html):
    ...
    for game in games:
        ...
        Priloshenie.objects.create(Name= game.find('div', class_='WsMG1c nnK0zc').get_text(strip=True) ,
        Linker= HOST + game.find('a', class_='JC71ub').get('href'),
        Price= content_price(game.find('span', class_='VfPpfd ZdBevf i5DZme')),
        Image = game.find('img', class_='T75of QNCnCf').get('data-srcset')
        [ :len(game.find('img', class_='T75of QNCnCf').get('data-srcset')) - 3])
